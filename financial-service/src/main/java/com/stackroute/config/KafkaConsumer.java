...
//        config.put(JsonDeserializer.VALUE_DEFAULT_TYPE,"com.stackroute.domain.Domain");
        return new DefaultKafkaConsumerFactory<>(config,new StringDeserializer(),new JsonDeserializer<>(TransactionDetails.class));
    }

    @Bean
    public ConcurrentKafkaListenerContainerFactory<String, TransactionDetails> kafkaListenerContainerFactory(){
        ConcurrentKafkaListenerContainerFactory<String, TransactionDetails> factory = new ConcurrentKafkaListenerContainerFactory();
